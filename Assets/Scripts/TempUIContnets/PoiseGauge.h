...
namespace Alice
{
    ALICE_DECLARE_DELEGATE_OneParam(FOnPoiseValueChanged, float);

    // 재사용 가능한 게이지 스크립트.
    // 에디터에서 rootWidgetName, gaugeWidgetName, targetEntityName, targetScriptName을 설정하여
    // 씬마다 이름만 바꿔서 재사용할 수 있습니다.
    class PoiseGauge : public IScript
    {
        ALICE_BODY(PoiseGauge);

    public:
        void Start() override;
        void Update(float deltaTime) override;

        // 델리게이트 (public으로 외부에서 접근 가능)
        FOnPoiseValueChanged OnPoiseValueChanged;

        // HP_Value 프로퍼티 (tmpTextScript 등에서 사용)
        ALICE_PROPERTY(float, HP_Value, 100.0f);
