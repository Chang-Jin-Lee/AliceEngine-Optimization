...
namespace Alice
{
    ...
    void PoiseGauge::Start()
    {
        ...
        {
            ALICE_LOG_WARN("[PoiseGauge] Root widget not found: %s", Get_rootWidgetName().c_str());
            return;
        }
        ...
        {
            ALICE_LOG_WARN("[PoiseGauge] Gauge widget not found: %s", Get_gaugeWidgetName().c_str());
            return;
        }
        ...
        {
            ALICE_LOG_WARN("[PoiseGauge] Gauge or Widget component not found: %s", Get_gaugeWidgetName().c_str());
            return;
        }
        ...
        if (!go.IsValid())
        {
            ALICE_LOG_WARN("[PoiseGauge] Target entity not found: %s", Get_targetEntityName().c_str());
            return;
        }
        ...
        for (auto& sc : *scripts)
        {
            ...
            {
                ALICE_LOG_INFO("[PoiseGauge] script on %s: %s (instance=%d)",
                    Get_targetEntityName().c_str(),
                    sc.scriptName.c_str(),
                    sc.instance ? 1 : 0);

                auto* box = static_cast<BoxDeligateScript*>(sc.instance.get());
                box->OnPoiseHPChanged.BindObject(this, &PoiseGauge::changeValue);
                // 초기값 반영 (바인딩 직후 델리게이트 실행)
                box->OnPoiseHPChanged.Execute(box->Get_PoiseHP_Value());
                break;
            }
        }
    }

    void PoiseGauge::changeValue(float newValue)
    {
        if (TargetGauge)
        {
            ...
            // OnPoiseValueChanged 델리게이트 실행
            OnPoiseValueChanged.Execute(newValue);
        }
    }
    ...
    float PoiseGauge::SmoothDamp(float current, float target, float& currentVelocity, float smoothTime, float dt)
    {
        float maxSpeed = 2.0f;
    ...
    void PoiseGauge::ExampleFunction()
    {
