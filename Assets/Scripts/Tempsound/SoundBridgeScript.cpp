...
namespace Alice
{
            ...
            case Combat::ActionState::Attack:
                if (bossBrain)
                {
                    ...
                    case C_BossBrainComponent::PatternType::AttackB:
                        {
                            // AttackB: 바로 1번, 애니메이션 중간 시점에 1번 (FMOD Delay 사용)
                            bus->RequestBossAttackSfx(BossAttackState::Attack1);
                            // 애니메이션 재생 시간 기반으로 딜레이 계산
                            float duration = GetBossPatternActualDuration(bossId, C_BossBrainComponent::PatternType::AttackB);
                            if (duration > 0.0f)
                            {
                                // 애니메이션 중간 시점에 재생 (직접 딜레이 지정)
                                float delay = duration * 0.5f;
                                bus->RequestBossAttackSfxDelayed(BossAttackState::Attack1, delay);
                            }
                            else
                            {
                                // 기본값 사용
                                bus->RequestBossAttackSfxDelayed(BossAttackState::Attack1, 1.0f);
                            }
                        }
                        break;
                    case C_BossBrainComponent::PatternType::AttackC:
                        {
                            // AttackC: 바로 1번, 1/3 시점, 2/3 시점에 각각 1번 (FMOD Delay 사용)
                            bus->RequestBossAttackSfx(BossAttackState::Attack1);
                            // 애니메이션 재생 시간 기반으로 딜레이 계산
                            float duration = GetBossPatternActualDuration(bossId, C_BossBrainComponent::PatternType::AttackC);
                            if (duration > 0.0f)
                            {
                                // 애니메이션 1/3, 2/3 시점에 재생 (각각 다른 딜레이로 같은 사운드 재생)
                                float delay1 = duration * (1.0f / 3.0f);
                                float delay2 = duration * (2.0f / 3.0f);
                                bus->RequestBossAttackSfxDelayed(BossAttackState::Attack1, delay1);
                                bus->RequestBossAttackSfxDelayed(BossAttackState::Attack1, delay2);
                            }
                            else
                            {
                                // 기본값 사용
                                bus->RequestBossAttackSfxDelayed(BossAttackState::Attack1, 1.0f);
                                bus->RequestBossAttackSfxDelayed(BossAttackState::Attack1, 2.0f);
                            }
                        }
                        break;
                    default:
    ...
    void SoundBridgeScript::Update(float deltaTime)
    {
        ...
        if (bossStateChanged || bossPatternChanged)
        {
            if (bossGo.IsValid())
            {
                OnCombatStateEntered(bossGo.id(), m_prevBossState, rawBossState, &bossFlags);
            }
            // 상태 전환 이벤트(Entered)에서만 prev 갱신
            if (bossStateChanged)
