...
> **팀 프로젝트가 끝난 뒤, 혼자 이어서 진행한 후속 개발 저장소입니다.**
>
> C++ · Direct3D 11 자체 엔진 **Alice Engine**의 렌더링 경로를 RenderDoc으로 다시 분석해
> 병목을 찾고, 계측 계층과 A/B 벤치마크를 직접 만들어 개선 폭을 수치로 검증했습니다.
>
> ### GPU 51.21 ms → 5.88 ms · **8.7배**
>
> 본 상수 버퍼 업로드 605 MB → 451 KB (1341배) · 같은 카메라 경로 598프레임 평균
>
> → [렌더링 최적화 상세](#렌더링-최적화--측정으로-검증한-87배)

| | |
|---|---|
| **원본 엔진 (팀 개발)** | [Chang-Jin-Lee/AliceEngine](https://github.com/Chang-Jin-Lee/AliceEngine) — 4인 공동 개발, 본인 기여 836 / 1,248 커밋 (약 67%) |
| **이 저장소 (개인 후속)** | 프로젝트 종료 후 단독 개발 — 병목 분석, 렌더 패스 정리, GPU 인스턴싱, 상수 버퍼 최적화, 계측·벤치마크 구축 |
...
| 항목 | 내용 |
|---|---|
| 저장소 성격 | 팀 프로젝트 종료 후 **개인 후속 개발** |
| 개선 결과 | GPU 51.21 ms → 5.88 ms (**8.7배**), 본 CB 업로드 1341배 감소 |
| 검증 방법 | 런타임 legacy 토글 + 카메라 테이크 재생 + 프레임별 CSV 계측 |
| 플랫폼 | Windows (Win32, Direct3D 11) |
| 엔진 | Alice Engine (자체 제작, C++20) — 엔진 코어 약 99,000줄 |
| 기술 스택 | `C++20` `Direct3D 11` `HLSL` `NVIDIA PhysX` `RTTR` `Assimp` `FMOD` `Dear ImGui` `CMake/vcpkg` |
| 원본 팀 프로젝트 | 2026.01.16 ~ 2026.02.13 (약 3주) · 8명 — 프로그래머 4 · 아티스트 2 · 기획 2 · 본인 팀 리더 |

---

## 렌더링 최적화 — 측정으로 검증한 8.7배

프로젝트에서 가장 크게 손본 부분입니다. 감이 아니라 **같은 장면을 두 번 재생해 수치로 비교**했습니다.

### 무엇이 문제였나

RenderDoc으로 배경 타일이 깔린 장면(13 × 26 = **338장**)을 캡처해서 세 가지를 찾았습니다.

| 발견 | 내용 |
|---|---|
| **드로우콜 3중 계상** | GBuffer에서 그린 불투명 오브젝트를 TransparentForward에서 다시 그리고, 기본 `outlineWidth`가 0이 아니라 아웃라인 패스에서 한 번 더 그렸습니다 |
| **정적 타일이 스키닝 경로로** | 움직이지 않는 배경 타일에 `BLENDINDICES` / `BLENDWEIGHT`가 붙어 스키닝 경로를 탔고, 드로우마다 **64KB 본 상수 버퍼를 Map/Unmap** 했습니다 |
| **인스턴싱 미적용** | 인스턴싱 조건이 `boneCount == 1`이라 `boneCount == 0`인 타일이 전부 탈락해, 같은 메시 수백 장을 개별 드로우로 냈습니다 |

### 어떻게 고쳤나

불투명/투명 필터링을 분리하고, 아웃라인 기본값을 0으로 바꾸고, 인스턴싱 조건을 완화했습니다. 그 밖에 본 상수 버퍼를 실제 본 수만큼만 채우기, 월드 행렬 계산의 힙 할당 제거, 정지한 포즈의 재계산 생략, 카메라 행렬 캐싱을 함께 적용했습니다.

### 어떻게 검증했나

"빨라졌다"는 주장은 조건이 다르면 무너집니다. 그래서 비교 조건을 고정했습니다.

- **계측 계층** — D3D11 타임스탬프 쿼리로 패스별 GPU 시간, 파이프라인 통계 쿼리로 드로우콜·PS 호출·정점 수를 수집
- **최적화 이전 경로를 런타임 토글로 복원** — 같은 빌드에서 `F10` 한 번으로 예전 코드 경로로 전환. 두 빌드를 비교하는 게 아니라 **한 빌드 안에서 경로만 바꿉니다**
- **카메라 테이크 재생** — 사람이 한 번 비행한 경로를 기록해 두고 양쪽 실행이 그대로 재생. 프레임 인덱스 기준으로 진행하므로 **두 실행의 프레임 번호가 정확히 일치**합니다
- 두 CSV 모두 워밍업을 제외한 **frame 302~899, 598줄로 동일**하게 기록됐습니다
...
프레임당 605MB를 올리던 것이 451KB가 됐습니다. 이 항목 하나가 개선 폭의 대부분을 설명합니다.
...
화면 우측 상단 HUD에 프레임 시간, 패스별 GPU 시간, 드로우콜, 업로드량이 실시간으로 표시됩니다. 배지에 `[LEGACY ON]` / `[CURRENT]`가 찍혀 어느 경로인지 영상만 봐도 구분됩니다.

### 남은 한계

- 프레임 시간 비교는 **GPU 시간 기준**입니다. 이후 실행은 창모드 표시 경로에서 60fps 상한에 걸려, 프레임 시간으로 재면 개선 폭이 실제보다 작게 보입니다
- `presentMs`는 엔진의 델타 타임 클램프(100ms) 때문에 최대 100ms에서 포화됩니다. 지표로 쓰지 않았습니다
- 드로우콜·PS 호출은 사전에 정한 게이트(2.0배 / 1.5배)를 넘지 못했습니다. 늘어난 이전 경로의 드로우가 대부분 깊이 테스트에서 걸러져 픽셀 작업으로 이어지지 않기 때문이며, 수치는 위 표에 그대로 실었습니다
...
Alice Engine이 실제로 게임을 돌린다는 증거입니다. 아래는 **원본 팀 프로젝트(2026.01~02)의 결과물**이며,
저는 팀 리더 프로그래머로서 카메라·락온, 애니메이션 블렌딩, Toon PBR 렌더링,
프로토타입 조립과 빌드/Git 운영을 담당했습니다.
이 저장소의 최적화 작업은 이 게임이 완성된 뒤에 시작했습니다.
