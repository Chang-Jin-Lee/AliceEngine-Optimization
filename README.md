...
위 두 장은 벤치 실행에서 뽑은 **같은 프레임, 같은 카메라 위치**입니다. 화면이 똑같다는 것을 보여주려고 실은 것입니다.
겹쳐 적은 51.2 ms와 5.9 ms는 저 두 프레임의 값이 아니라 아래 표와 같은 598프레임 평균입니다.
PNG 덤프는 프레임 시간을 왜곡하므로 계측 실행과 분리해서 돌렸고 그래서 이 두 프레임에는 측정된 GPU 시간이 없습니다.
