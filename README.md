...
컴포넌트를 한 곳에 모아 순회하는 구조가 실제로 유리한지 같은 워크로드로 재봤습니다. `step` 격차는 데이터가 전부 L1에 들어가는 N=100에서도 이미 약 3배인데, 이건 캐시가 아니라 간접 참조와 가상 호출 비용입니다. 캐시가 만드는 몫은 그 위에 규모와 파편화로 붙는 증분이고, 파편화 상태에서는 8배 가까이 벌어집니다. 하드웨어 캐시 카운터로 다시 재도 같은 방향이 나왔습니다.
