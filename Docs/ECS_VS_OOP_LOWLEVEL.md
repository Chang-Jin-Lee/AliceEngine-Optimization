...
"저장은 Sparse Set으로 캐시 효율을 높입니다"와 `Docs/SPARSE_SET_USAGE_EXAMPLES.md:8, 112`의
"캐시 효율 극대화"/"캐시 미스 최소화"다. 같은 취지의 주장을 포트폴리오 페이지에도 적어뒀었는데,
면접에서 "그걸 정말 측정해봤느냐"는 질문을 받았다. Task 6의 시간 측정은 그 주장의 **결과**(더 빠르다)는 보여주지만
...
마지막 샘플의 차(delta)**를 구했다(자세한 방법론은 뒤의 한계 참고). 표본 개수는 5초 동안 그 스레드가 컨텍스트
...
5. **LLC 미스/참조 비율(%)은 N=1,000 구간에서 신뢰도가 낮다** (위 해석 참고) — 분모(LLC 참조 수)
