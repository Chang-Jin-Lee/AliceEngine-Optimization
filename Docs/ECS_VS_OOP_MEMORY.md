...
결론부터. 누수가 아니라 씬 로드 중에 끝나는 유한한 비용이었고 엔티티당 8.0MiB가 들었다.
원인은 `FbxAnimation::PrecomputeAll`이 엔티티마다 도는 것이었다. 메시별 1회로 고쳐
**40,335MiB가 693MiB로, 로딩이 약 110초에서 15초로** 줄었다.

원인에 닿기까지 가설을 여럿 세웠다 폐기했고 도구에서도 여러 번 막혔다. 그 과정을 그대로 남긴다.
같은 함정을 다시 밟지 않으려는 목적이 크다.
...
호출 지점을 찾는 데 세 번 실패했다. 매번 스택에서 "첫 프레임"을 너무 이르게 받아들인 탓이다.

1. `MAINPATH` 소스만 필터링 → 344만 건 중 25만 건(7%)만 집계. 아무것도 비례하지 않는 것처럼 보였다.
2. 모듈 필터가 `ntdll`만 → `ucrtbase.dll`에서 멈춤. "CRT 할당자가 범인"이라는 무의미한 답.
3. `std::vector<...>::_Resize_reallocate`가 Launch.exe 안에 인스턴스화돼 있어 통과 →
   벡터 메서드가 범인처럼 보임.

`std::`로 시작하는 프레임까지 전부 건너뛰고서야 실제 호출자가 나왔다.

| 할당량 | 건수 | 지점 | 엔티티당 |
|---:|---:|---|---:|
| 2,408 MiB | 344,400 | `FbxAnimation::EvaluateGlobals` | 1,148건 |
| 2,313 MiB | 173,700 | `FbxAnimation::PrecomputeAll` | **579건** |

`PrecomputeAll`은 모든 애니메이션 클립을 초당 30샘플로 구워 팔레트를 만든다.
이게 **엔티티마다** 돌고 있었다. 32 엔티티에서 17,220건, 302 엔티티에서 172,200건으로 정확히 10배다.
트레이스 길이와 무관하게 엔티티당 579건으로 고정이라는 점이 로드 시 1회성임을 말해준다.

### 캐시가 비어 있던 이유

레지스트리에 `GetPrecomputedAnimation`/`SetPrecomputedAnimation` 캐시가 이미 있었고
`SkinnedAnimationSystem`과 `AnimBlueprintSystem`은 그걸 쓰고 있었다. 문제는 두 가지였다.

1. **캐시를 채우는 `PrecomputeAnimationForMesh`가 프리로드 경로에서만 호출된다.**
   씬 파일로 들어온 메시는 온디맨드 임포트 경로(`EnsureSkinnedMeshesRegisteredForWorld`)를 타는데
   거기서는 캐시를 채우지 않았다. 그래서 캐시는 늘 비어 있었다.
2. **`AdvancedAnimSystem`은 캐시를 조회하지도 않았다.** 형제 시스템 둘과 달리
   `CopyPrecomputedFrom` 호출이 빠져 있었다.
3. 그리고 `CopyPrecomputedFrom` 자체가 깊은 복사였다.
   `m_Precomputed`가 `vector<PrecomputedClip>`이고 각 클립이 `vector<vector<XMMATRIX>> palettes`라
   샘플마다 할당이 하나씩 생겼다. 캐시가 채워졌더라도 복사 비용이 그대로 엔티티당 비용이 됐을 것이다.

### 수정

- 온디맨드 임포트 직후 프리컴퓨트를 한 번 구워 레지스트리에 올린다
  (`Engine::Impl::EnsurePrecomputedAnimationForMesh`). 프리로드 경로가 하던 일을 같이 하게 만든 것이다.
- `AdvancedAnimSystem`이 캐시를 조회하도록 `CopyPrecomputedFrom` 호출을 추가했다.
- `FbxAnimation::m_Precomputed`를 `shared_ptr<const vector<PrecomputedClip>>`으로 바꿨다.
  구워진 뒤에는 읽기만 하므로 공유가 안전하고 `CopyPrecomputedFrom`이 O(1) 포인터 대입이 된다.

### 결과

| | 수정 전 | 수정 후 |
|---|---:|---:|
| 302 엔티티 피크 워킹셋 | 2,758 MiB | **685 MiB** |
| 302 엔티티 커밋 | 4,029 MiB | **1,490 MiB** |
| **5,002 엔티티 피크 워킹셋** | **40,335 MiB** | **693 MiB** |
| 5,002 엔티티 로딩(안정까지) | 약 110초 | **약 15초** |
| 프리컴퓨트 실행 | 엔티티마다 | 메시당 1회 |

**엔티티 수에 대한 비례가 사라졌다.** 5,002 엔티티가 302 엔티티와 같은 메모리를 쓴다.

렌더링과 애니메이션이 그대로인지 확인했다. `#01PrototypeMap` 6,464프레임에서
드로우콜 6,700, 인스턴스 드로우 58, 본 상수 버퍼 업로드 451,056 B/프레임으로
문서에 적힌 최적화 후 수치와 일치한다. 본 버퍼 업로드량이 정확히 같다는 것은
본 팔레트가 동일하게 계산된다. `RenderMetricsTests`도 통과한다.
