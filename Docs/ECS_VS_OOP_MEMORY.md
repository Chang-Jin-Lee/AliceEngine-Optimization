...
로드 경로 전체의 할당 패턴이 만드는 파편화이기 때문이다. 실질적인 대응은 셋 중 하나다.

1. 씬 로드의 엔티티별 임시 할당을 아레나/풀로 묶어 힙을 건드리지 않게 한다. 근본 해결에 가깝다.
2. 로드가 끝난 뒤 `_heapmin()`으로 힙을 한 번 정리한다. 효과는 부분적이고 확인이 필요하다.
3. 그대로 둔다. 상주 메모리가 아니라 커밋이고, 로드 후에는 더 늘지 않는다.
   다만 5,000 엔티티에서 40GB는 128GB 미만 기계에서 문제가 된다.
