...
- [x] `Build.bat`이 성공하고 경고가 새로 늘지 않습니다.
- [x] 에디터를 띄운 뒤 `GpuProfiler::ScopeMs(GpuScope::Frame)`이 0보다 큰 값을 반환합니다. 임시 로그로 확인하고 로그는 지웁니다.
- [x] 패스별 스코프 ms 합이 `Frame` 스코프 ms의 0.8배에서 1.2배 사이입니다. 벗어나면 스코프가 빠졌거나 겹친 것입니다.
- [x] `drawCalls`가 0이 아니고, 씬에 오브젝트를 추가하면 값이 늘어납니다.
- [x] 계측을 끈 상태와 켠 상태의 평균 `presentMs` 차이가 3% 이내입니다.
- [x] `Disjoint`가 참인 프레임을 폐기하는 경로에 도달하는지 로그로 한 번 확인합니다. GPU 클럭이 안정적이면 안 나올 수 있으므로, 강제로 참을 넣어 폐기 분기가 도는 것만 확인하고 되돌립니다.

## 검증 결과

- Release 에디터 정상 구간 180프레임에서 GPU `Frame`은 최소 26.028032ms, 평균 45.713806ms였습니다.
- 같은 구간의 패스 합/프레임 비율은 최소 0.977914, 평균 0.999251, 최대 0.999977이었습니다.
- SampleScene의 `drawCalls`는 23이었고, 서로 다른 머티리얼 오브젝트를 임시 추가하면 24로 증가했습니다.
- 같은 장면의 600프레임 평균 `presentMs`는 계측 켜짐 45.925468ms, 꺼짐 45.982629ms로 차이가 0.124309%였습니다.
- 120프레임 이후 disjoint를 강제했을 때 폐기 카운터가 98까지 증가했고, 마지막 정상 GPU 값 43.621376ms는 바뀌지 않았습니다.
- 임시 오브젝트, 강제 disjoint, 진단 로그는 모두 제거한 뒤 `Build.bat`, Release 대상 빌드, CTest를 다시 통과했습니다.
